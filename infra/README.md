...
Provisions the **static-site** hosting for zalizniak.com:

- **Amplify app** (`platform = "WEB"`) — builds the Astro/Starlight site from
  GitHub (`aillusions/zalizniak.com`) on every push to `main` and serves
  `dist/` behind CloudFront.
- **Custom domain** — serves at the apex `zalizniak.com` **and** `www.zalizniak.com`,
  with the ACM-validation + apex/www alias records published into the existing
  `zalizniak.com` Route 53 zone.
...
`zalizniak.com/terraform.tfstate`.

## Prerequisites

- AWS credentials with admin-ish rights in account `116085141062`.
- The `zalizniak.com` hosted zone already exists in Route 53 (this code looks
  it up; it doesn't create it).
- Terraform >= 1.10.
...
- `web_url` — `https://zalizniak.com`, live a few minutes after the cert verifies.
- `amplify_console_url` — deep link to watch builds.

## Zero-downtime cutover (apex already serving elsewhere)

Because the apex is live, apply in stages so traffic only flips once the new
