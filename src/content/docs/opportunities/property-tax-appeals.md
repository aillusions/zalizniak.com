...
Why it stayed a boutique cottage industry: the work is deadline-bound and fragmented across thousands of assessing jurisdictions with their own forms, calendars, and evidence norms — exactly the per-jurisdiction arcana that resists generic software ([Opportunities thesis](/opportunities/about/)). That fragmentation is the moat; an agent that encodes it scales where a consultant's billable hours can't.
...
This is the playbook in miniature: [an agent that acts on the deadline rather than waiting to be asked](/ai-playbook/agent-assistant-to-actor/); [per-jurisdiction rules — deadlines, forms, evidence norms — encoded as a domain layer](/ai-playbook/encoding-domain-rules/) instead of hand-held per county; and [pulling assessor rolls and comps from systems with no API](/ai-playbook/integrating-systems-without-apis/).
...
Reconstructed from public sources; claims are tier-labeled (VERIFIED / INFERRED / SPECULATIVE) — see [how to read the tiers](/opportunities/about/). Supporting quotes live in this repo's evidence map (`evidence/opp-property-tax-appeals-evidence-map.md`).
