...
This is the playbook in miniature: [an agent that acts on the deadline rather than waiting to be asked](/playbook/agent-assistant-to-actor/); [per-jurisdiction rules — deadlines, forms, evidence norms — encoded as a domain layer](/playbook/encoding-domain-rules/) instead of hand-held per county; and [pulling assessor rolls and comps from systems with no API](/playbook/integrating-systems-without-apis/).
