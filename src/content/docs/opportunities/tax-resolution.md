...
This is the playbook in miniature: [an agent that acts on the deadline rather than waiting to be asked](/ai-playbook/agent-assistant-to-actor/); [the IRS's notice types, deadlines, and remedy-eligibility rules encoded as a domain layer](/ai-playbook/encoding-domain-rules/); and [reading IRS transcripts and a taxpayer's own records that mostly lack a clean API](/ai-playbook/integrating-systems-without-apis/).
