...
This is the playbook in miniature: [an agent that acts on the deadline rather than waiting to be asked](/ai-playbook/agent-assistant-to-actor/); [Article 9's name, jurisdiction, and timing rules encoded as a domain layer](/ai-playbook/encoding-domain-rules/); and [reading Secretary-of-State organic records and filing-office search systems that mostly expose no clean API](/ai-playbook/integrating-systems-without-apis/).
