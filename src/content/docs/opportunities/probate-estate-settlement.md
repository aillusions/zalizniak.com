...
This is squarely the playbook's territory: [an agent that acts, not suggests](/playbook/agent-assistant-to-actor/); a [durable, long-running workflow](/playbook/durable-long-running-workflows/) that can't lose state across 16 months; [integration with systems that have no API](/playbook/integrating-systems-without-apis/) (portals, mail, phone trees); and [per-jurisdiction rules encoded as a domain layer](/playbook/encoding-domain-rules/) rather than hand-held per county.
