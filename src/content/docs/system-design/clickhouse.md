...
It's not a transactional store: no multi-row ACID, eventually-consistent replication, async mutations — all the things you'd lean on [Postgres](/system-design/postgres-internals/) for.
...
**Avoid it for:** OLTP, frequent updates/deletes, point lookups by key (a [row store](/system-design/postgres-internals/) or [KV](/system-design/cassandra/) fits), per-row transactions, or high-concurrency single-record reads. It trades flexible writes and transactions for raw analytical scan speed.

---

These are working notes — ClickHouse as the columnar/OLAP point in the data-store space, beside row-oriented [Postgres](/system-design/postgres-internals/) and wide-column [Cassandra](/system-design/cassandra/). The throughline: columnar layout + sorted parts + sparse index + vectorized execution is *why* it scans billions of rows fast — and why it's the wrong tool for a single-row update.
