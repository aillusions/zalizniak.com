...
This is the playbook in miniature: [an agent that acts on the deadline rather than waiting to be asked](/archive/ai-playbook/agent-assistant-to-actor/); [the IRS's notice types, deadlines, and remedy-eligibility rules encoded as a domain layer](/archive/ai-playbook/encoding-domain-rules/); and [reading IRS transcripts and a taxpayer's own records that mostly lack a clean API](/archive/ai-playbook/integrating-systems-without-apis/).
...
Reconstructed from public sources; claims are tier-labeled (VERIFIED / INFERRED / SPECULATIVE) — see [how to read the tiers](/archive/opportunities/about/). Supporting quotes live in this repo's evidence map (`evidence/opp-tax-resolution-evidence-map.md`).
