...
The frontier moves faster than any training budget: a model you fine-tune today is competing with next quarter's general model, so a custom model is a depreciating asset unless your data gives an edge the labs can't get. But renting isn't free either — frontier calls on every step are expensive (see [Keeping inference cheap & fast](/archive/ai-playbook/inference-cost-and-latency/)), some tasks are saturated enough that a cheap open-source model matches frontier quality, and regulated domains have constraints — zero data retention, conflict-aware governance — that no managed API offers. So "own vs. rent" isn't one decision; it's a per-capability call weighing data moat, cost, latency, control, and compliance.

## Patterns

**Rent the reasoning, own the data** — Treat frontier reasoning as a swappable commodity and put your investment into the proprietary data and pipeline around it. [Rilla](/archive/teardowns/rilla/) owns custom field-noise ASR (PyTorch + Baseten) over a corpus of "millions of in-person conversations no competitor captures," but rents OpenAI reasoning through a LiteLLM router, kept swappable; [Glean](/archive/teardowns/glean/) owns the index, knowledge graph, and permissions — the moat — and keeps models rented and zero-retention. — [Rilla](/archive/teardowns/rilla/), [Glean](/archive/teardowns/glean/)

**Fine-tune only where your data beats the frontier** — Train or fine-tune a vertical model where proprietary data gives an edge a general model can't match, and rent everything else. [Pallet](/archive/teardowns/pallet/) fine-tunes "Pallet Core" on "licensed supply-chain datasets" via Vertex AI for logistics patterns while renting frontier reasoning; [Confido](/archive/teardowns/confido/) is building CPG-tuned proprietary models because the domain is underserved by general ones. — [Pallet](/archive/teardowns/pallet/), [Confido](/archive/teardowns/confido/)

**Stay model-agnostic: route by benchmark, not allegiance** — Make the model a routed dependency chosen by a scored internal benchmark re-run each release, so a better model is an upgrade, not a migration. [Basis](/archive/teardowns/basis/) is "model-agnostic by benchmark, not allegiance" — GPT-5 "hit a perfect 100% success rate on its parallel-tool-calling benchmark before being promoted to the supervisor role." — [Basis](/archive/teardowns/basis/)

**Self-host open-source to cut cost on saturated tasks** — Where open-source matches frontier quality on a task, route there and keep frontier for the hard reasoning; owning the runtime makes that tenable. [Harvey](/archive/teardowns/harvey/): "the choice of model is just a routing decision" behind a Legal Agent Benchmark quality bar, with measured 3–5x cost reductions routing intelligence-saturated tasks to self-hosted open-source models. — [Harvey](/archive/teardowns/harvey/)

**Own the runtime when compliance forces it** — Sometimes you own infrastructure not for the model but for guarantees no managed platform offers — zero data retention, conflict-aware governance, access enforced at the DB layer. [Harvey](/archive/teardowns/harvey/) owns its agent runtime precisely because "automatic state persistence and zero retention are mutually exclusive," and privileged legal work needs both durability and ZDR. — [Harvey](/archive/teardowns/harvey/)

## Tools & popular choices

| Decision | Common choice | Notes |
| --- | --- | --- |
| Reasoning | Rent frontier via a provider-agnostic router | [Rilla](/archive/teardowns/rilla/) via LiteLLM; [Glean](/archive/teardowns/glean/) and [Basis](/archive/teardowns/basis/) multi-provider — swap a model in roughly one line. |
| Domain perception (speech / vision / extraction) | Own / fine-tune on proprietary data | [Rilla](/archive/teardowns/rilla/) custom ASR on PyTorch + Baseten; the noisy domain data is the edge. |
| Vertical reasoning model | Fine-tune a base model on licensed/proprietary data, only where it beats frontier | [Pallet](/archive/teardowns/pallet/) Core via Vertex AI on supply-chain data; [Confido](/archive/teardowns/confido/) building CPG-tuned models. |
| Cost on high-volume tasks | Self-host open-source (vLLM/TGI on K8s), route by benchmark | [Harvey](/archive/teardowns/harvey/): 3–5x cheaper on saturated tasks behind a quality bar. |
| Model selection | A scored internal benchmark, re-run per release | [Basis](/archive/teardowns/basis/) promotes models by eval, not vendor relationship. |
| The real moat | Data, retrieval, runtime, integrations — not the model | [Glean](/archive/teardowns/glean/) (retrieval + permissions), [Pallet](/archive/teardowns/pallet/) (memory), [Confido](/archive/teardowns/confido/) (50+ connectors). |
...
- **Route by benchmark, not allegiance.** A scored internal eval re-run each release lets a better model earn promotion without a rewrite (see [Testing output that isn't reproducible](/archive/ai-playbook/evaluating-non-deterministic-agents/)).
...
- [Rilla](/archive/teardowns/rilla/) — owns custom field-noise ASR (PyTorch + Baseten) over a proprietary conversation corpus no competitor captures, but rents frontier reasoning via LiteLLM, kept swappable.
- [Basis](/archive/teardowns/basis/) — pure rent, model-agnostic by benchmark: each step routed to the best-fit OpenAI model off a scored internal suite re-run every release (GPT-5 promoted to supervisor after a perfect benchmark).
- [Harvey](/archive/teardowns/harvey/) — owns the runtime for compliance (zero retention, conflict-aware governance) and routes by a Legal Agent Benchmark, self-hosting open-source for 3–5x cost cuts on saturated tasks.
- [Pallet](/archive/teardowns/pallet/) — fine-tunes a vertical model (Pallet Core) on licensed supply-chain data via Vertex AI for logistics patterns, while renting frontier reasoning and keeping per-customer uniqueness in a memory layer.
- [Glean](/archive/teardowns/glean/) — owns the retrieval engine, knowledge graph, and permissions (the moat) and rents models, kept multi-provider and zero-retention.
