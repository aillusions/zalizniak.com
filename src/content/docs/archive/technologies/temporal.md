...
slug: archive/system-design/temporal
...
**Avoid it for:** simple request/response, single fire-and-forget tasks (a [queue](/archive/system-design/kafka/#event-vs-task--kafka-vs-a-queue) is lighter), pure event fan-out (that's [Kafka](/archive/system-design/kafka/)), or ultra-low-latency hot paths. It's an orchestration layer, not a message bus or a job runner.
...
These are working notes on durable execution. The one idea to keep: **persist every step so the process survives anything, and make one thing own the flow** — which is what separates orchestration from a pile of events and glue. Vocabulary in the [Terminology](/archive/system-design/terminology/#async--messaging); the data-vs-work contrast on the [Kafka page](/archive/system-design/kafka/#kafka-vs-temporal--moving-data-vs-moving-work).
