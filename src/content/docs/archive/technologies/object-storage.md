...
slug: archive/system-design/object-storage
...
Object storage is the **blob layer** of almost every system — images, video, backups, logs, data-lake files, [WAL](/archive/system-design/postgres-internals/#wal-write-ahead-log) archives for [PITR](/archive/system-design/postgres-internals/#wal-write-ahead-log). The reference is **Amazon S3** (its API is the de-facto standard); **MinIO** is the open-source, S3-compatible implementation you can self-host, read, and break — so it's the vehicle here for *how object storage is actually built*. It completes the data-store shelf: relational ([Postgres](/archive/system-design/postgres-internals/)) · wide-column ([Cassandra](/archive/system-design/cassandra/)) · columnar ([ClickHouse](/archive/system-design/clickhouse/)) · **object (this)**.
...
- **[Presigned URLs](/archive/system-design/terminology/#data--storage)** — a time-limited signed URL lets a client upload/download *directly* to the store, keeping big files off your app servers (the upload/download path most designs reach for). Pair with a **CDN** for read fan-out.
...
**Avoid it for:** small, low-latency, frequently-mutated records (a [database](/archive/system-design/postgres-internals/) or [KV](/archive/system-design/cassandra/) store fits); anything needing partial in-place updates or transactions; or as a general filesystem (no efficient rename/append, listing is not free).
