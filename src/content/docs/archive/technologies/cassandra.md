...
slug: archive/system-design/cassandra
...
- **Last-write-wins (LWW)** — conflicts resolve by each cell's **timestamp**, so **[clock skew](/archive/system-design/terminology/#data--storage) across nodes can silently drop a write**. (Modern Cassandra uses LWW, not vector clocks.)
...
**Avoid it for:** ad-hoc queries, joins, or aggregations (it isn't relational — reach for [Postgres](/archive/system-design/postgres-internals/)); workloads needing strong consistency or multi-key transactions by default; small datasets where the operational weight (repair, compaction tuning, capacity planning) isn't worth it.

---

These are working notes — Cassandra as the concrete embodiment of leaderless, quorum-based ideas, with the one-liner terms (quorum, consistent hashing, eventual consistency) in the [Terminology](/archive/system-design/terminology/#data--storage). The throughline: trade joins and default-strong consistency for linear write scale and no single point of failure.
