...
The layer *above* the transports ([Kafka](/archive/system-design/kafka/), [queues](/archive/system-design/queues/), [Temporal](/archive/system-design/temporal/)): how services actually talk. In **event-driven architecture (EDA)** — the standard name for this — services communicate by **asynchronous messages** instead of direct synchronous calls, which buys loose coupling, isolation, and resilience (a slow or dead consumer doesn't take the producer down).
...
Same messages on the wire; the difference is [intent](/archive/system-design/terminology/#async--messaging). (The Reactive Manifesto calls the underlying async-messaging foundation "message-driven" — a less-common synonym you'll occasionally see.)
...
- **[Event sourcing](/archive/system-design/terminology/#distributed-transactions--consistency)** — the event log *is* the source of truth; state is a fold over events. Full audit + replay, harder to query.
- **[CQRS](/archive/system-design/terminology/#distributed-transactions--consistency)** — split the write model from read models kept in sync via events; often paired with event sourcing.
...
| --- | --- | --- | --- |
| Event log / streaming backbone | **[Kafka](/archive/system-design/kafka/)** | Kinesis (AWS-native), Pulsar (tiered storage, multi-tenant), Redpanda (Kafka-API, lower latency) | replay, multiple independent consumers, high throughput, event sourcing |
| Task / work queue | **[SQS](/archive/system-design/queues/)** (managed) / RabbitMQ | — | point-to-point, competing consumers, "do this job once," decoupled offload |
...
- **[Event sourcing + CQRS](/archive/system-design/terminology/#distributed-transactions--consistency)** — when the domain genuinely wants an audit log and split read/write models; don't reach for it by default.
...
- **Costs:** **eventual consistency** (no synchronous "it's done"), much harder **debugging and observability** (a request's path is spread across services and time — lean on [tracing](/archive/system-design/observability/#tracing)), ordering and duplicate handling ([idempotency](/archive/system-design/terminology/#async--messaging)), and — in pure choreography — **no single owner** of the flow.
...
These are working notes — the architecture that composes the messaging transports. The throughline: async messages decouple services; *events* let many react, *commands* direct one, and the choreography-vs-orchestration choice decides **who owns the flow**. Transports on the [Kafka](/archive/system-design/kafka/), [Queues](/archive/system-design/queues/), and [Temporal](/archive/system-design/temporal/) pages; vocabulary in the [Terminology](/archive/system-design/terminology/#async--messaging).
